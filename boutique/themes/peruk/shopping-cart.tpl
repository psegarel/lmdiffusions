...
<div id="order-detail-content" class="table_block">
	<table id="cart_summary" class="std" cellspacing="0" cellpadding="0">
		<thead>
			<tr>
				<th class="cart_product first_item">{l s=''}</th>
				<th class="cart_description item">{l s='Product'}</th>
				...
				<th class="cart_total last_item">{l s='Sous Total'}</th>
				<th class="trash"> </th>
			</tr>
		</thead>
		<tfoot>
					...
					{if $voucherAllowed}
						...
						<form action="{$smarty.server.PHP_SELF|htmlentities}" method="post" id="voucher">
							<p>
								...
								<span class="submit"><input type="submit" name="submitDiscount" value="{l s='Add'}" class="button" /></span>
							</p>
						</form>
					{/if}
				</td>
			...
			<tr class="cart_total_product">
				<td colspan="4" class="price">{l s='Total products:'}</td>
				<td colspan="2" class="price">{convertPrice price=$total_products_wt}</td>
			</tr>
			-->
			{if $total_discounts != 0}
			<tr class="cart_total_voucher">
				<td colspan="4" class="price">{l s='Total vouchers:'}</td>
				<td colspan="2" class="price-discount">{convertPrice price=$total_discounts}</td>
			</tr>
			{/if}
			{if $total_wrapping > 0}
			<tr class="cart_total_voucher">
				<td colspan="4" class="price">{l s='Total gift-wrapping:'}</td>
				<td colspan="2" class="price-discount">{convertPrice price=$total_wrapping}</td>
			</tr>
			{/if}
			{if $shippingCost > 0}
			<tr class="cart_total_delivery">
				<td colspan="4" class="price">{l s='Total shipping:'}</td>
				<td colspan="2" class="price">{convertPrice price=$shippingCost}</td>
			</tr>
			{/if}
			<tr class="cart_total_price">
				<td colspan="4" class="price">{l s='Total:'}</td>
				<td colspan="2" class="price">{convertPrice price=$total_price}</td>
			</tr>
			{if $free_ship > 0}
			<tr class="cart_free_shipping">
				<td colspan="4" class="price" style="white-space: normal;">{l s='Remaining amount to be added to your cart in order to obtain free shipping:'}</td>
				<td colspan="2" class="price">{convertPrice price=$free_ship}</td>
			</tr>
			{/if}
		</tfoot>
		<tbody>
