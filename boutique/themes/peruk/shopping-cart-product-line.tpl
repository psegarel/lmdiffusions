<tr class="{if $smarty.foreach.productLoop.last}last_item{elseif $smarty.foreach.productLoop.first}first_item{/if}{if isset($customizedDatas.$productId.$productAttributeId) AND $quantityDisplayed == 0}alternate_item{/if} cart_item">
	...
	<td class="cart_quantity">
		...
		<p>
			{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}<span class="la_quantite">{$customizedDatas.$productId.$productAttributeId|@count}</span>{else}<span class="la_quantite">{$product.quantity-$quantityDisplayed}</span>{/if}
			<a class="cart_quantity_up" href="{$base_dir}cart.php?add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;token={$token_cart}" title="{l s='Add'}"><img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Add'}" /></a>
