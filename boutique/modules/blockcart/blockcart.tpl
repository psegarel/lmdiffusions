...
{if $ajax_allowed}
<script type="text/javascript" src="{$base_dir}js/jquery/iutil.prestashop-modifications.js"></script>
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				...
				<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}" title="{$product.name|escape:htmlall:'UTF-8'}">{$product.name|escape:htmlall:'UTF-8'}</a>
...
<div id="informations_block_nuancier" class="block">
	<a href="/boutique/category.php?id_category=301" title="{l s='Nuancier'}" target="_self">
	<img src="{$img_dir}/nuancier.png" alt="{l s='Nuancier'}" class="middle" />
	</a>
</div>
