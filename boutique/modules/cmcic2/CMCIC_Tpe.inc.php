...
define("CMCIC_URLPAIEMENT", Configuration::get('CMCIC_SERVEUR'));
...
class CMCIC_Tpe_nf {
	...
	function __construct($sLangue = "FR") {
			...
			// contrôle de l'existence des constantes de paramétrages.
			$aRequiredConstants = array('CMCICNF_CLE', 'CMCIC_VERSION', 'CMCICNF_TPE', 'CMCICNF_CODESOCIETE');
			$this->_checkTpeParams($aRequiredConstants);

			$this->sVersion = CMCIC_VERSION;
			$this->_sCle = CMCICNF_CLE;
			$this->sNumero = CMCICNF_TPE;
			$this->sUrlPaiement = CMCICNF_SERVEUR . CMCIC_URLPAIEMENT;

			$this->sCodeSociete = CMCICNF_CODESOCIETE;
			$this->sLangue = $sLangue;
