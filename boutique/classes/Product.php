...
class		Product extends ObjectModel
{
	...
	*/
	 ...
	 */
	public function updatePosition($way)
	{
		if (!$res = Db::getInstance()->ExecuteS('SELECT cp.`id_product`, cp.`position`, cp.`id_category` FROM `'._DB_PREFIX_.'category_product` cp
		...
		return (Db::getInstance()->Execute('
			...
			WHERE `'.pSQL($this->identifier).'` = '.intval($from[$this->identifier]).' AND `id_category`='.intval($from['id_category']))
			AND
			Db::getInstance()->Execute('
			UPDATE `'._DB_PREFIX_.'category_product`
			SET `position`='.intval($from['position']).'
			WHERE `'.pSQL($this->identifier).'` = '.intval($to[$this->identifier]).' AND `id_category`='.intval($to['id_category']))
			);
	}
	...
	*/
	...
	public function	getDocuments($id_lang)
	{
		return Db::getInstance()->ExecuteS('
		SELECT `id_document`,`doc_iso_code`, `legend`
		FROM `'._DB_PREFIX_.'documents` 
		WHERE `id_product` = '.intval($this->id).($id_lang ? ' AND `doc_iso_code` = '._USER_ID_LANG_ : intval($cookie->id_lang)).'
		ORDER BY `position`');
	}
		public function	getFlashs($id_lang)
	{
		return Db::getInstance()->ExecuteS('
		SELECT `id_flash`,`flash_iso_code`, `legend`
		FROM `'._DB_PREFIX_.'flashs` 
		WHERE `id_product` = '.intval($this->id).($id_lang ? ' AND `flash_iso_code` = '._USER_ID_LANG_ : intval($cookie->id_lang)).'
		ORDER BY `position`');
	}
	/**
	* Get new products
	...
	*/
	...
	static public function getProductProperties($id_lang, $row)
	{
		...
		$row['attribute_price'] = $row['id_product_attribute'] ? Product::getProductAttribute($row['id_product_attribute']) : 0;
