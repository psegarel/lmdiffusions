...
else
{
	...
	else
	{
		...
		// Ajout-PDF
		$documents = $product->getDocuments(intval($cookie->id_lang));
    	$smarty->assign('pdf', $documents);
		// FIN Ajout-PDF
		
		// Ajout-fichiers FLASH
		$flashs = $product->getFlashs(intval($cookie->id_lang));
    	$smarty->assign('swf', $flashs);
		// FIN Ajout-fichiers FLASH
