...
function	changeLanguageDocument(field, fieldsString, id_language_new, iso_code)
{
	var fields = fieldsString.split('¤');
	for (var i = 0; i < fields.length; ++i)
	{
		getE('languages_current_' + fields[i]).src = '../img/l/' + id_language_new + '.jpg';
		getE('language_current_' + fields[i]).value = id_language_new ;
	}
 	getE('languages_' + field).style.display = 'none';
	id_language = id_language_new;
}
